...
- Compose prepared datasets through topic detection and summarization with explicit provider-neutral input bounds and off-thread database preparation.
- Compose validated analysis-run results into deterministic, KST-dated Markdown editions with evidence and content-free source provenance.
...
The implemented system covers collection, raw-data preservation, message lifecycle handling, language detection, scoped analysis and statistics, and the automatic translation producer/queue/worker/storage pipeline. Analysis dataset preparation resolves current translations in a batch and falls back to clearly marked raw source text when a translation is missing; stale translations are preserved but never selected. A provider-independent analysis-run service composes prepared datasets through validated topic detection and evidence-linked summarization. It applies caller-supplied message and prepared-text character limits before provider calls and performs synchronous dataset preparation outside the async caller thread. A pure composer now turns validated run results into deterministic Markdown editions without exposing prepared message text. No production detector, summarizer, runtime wiring, topic/summary persistence, newspaper file storage, or Discord delivery has been selected. The queue is in-memory with no restart recovery or automatic retry. Live Discord and external-provider validation on a new host requires an explicit operational approval because it can contact external services and incur cost.
